...
export class ResturantsStore {
    ...
    async create(rest: Resturant):Promise<Resturant>{
        try {
            const conn = await client.connect();
            const sql = 'INSERT INTO resturants (rest_name, rest_location, price_range) VALUES ($1, $2, $3) RETURNING *;';
            const result = await conn.query(sql, [rest.rest_name, rest.rest_location, rest.price_range]);
            conn.release();
    ...
    async update(rest: Resturant):Promise<Resturant>{
        try {
            const conn = await client.connect();
            const sql = 'UPDATE resturants SET rest_name=$1, rest_location=$2, price_range=$3 WHERE id=$4 RETURNING id, rest_name, rest_location, price_range;'
            const result = await conn.query(sql, [rest.rest_name, rest.rest_location, rest.price_range, rest.id]);
            conn.release();
            return result.rows[0];
        } catch (error) {
    ...
    async delete(id: number):Promise<Resturant>{
        try {
            const conn = await client.connect();
            const sql = 'DELETE FROM resturants WHERE id=$1 RETURNING rest_name, rest_location, price_range;';
            const result = await conn.query(sql, [id]);
            conn.release();
            return result.rows[0];
        } catch (error) {
